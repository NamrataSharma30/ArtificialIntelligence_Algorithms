...
def adjacent_nodes(cost, current_node, queue, visited, goal):  # to calculate the nearest neighbor of the node
    child = []
    ...
    if c > 0:
        left = (r, c - 1)
        if cost[left[0], left[1]] != 0 and left not in visited:  # checks if a node is already visited; not to pass 0(impassable terrain)
            queue.append(left)
            child.append(left)
    ...
    BFS(cost, queue, visited, goal)  # calls the method with the expanded nodes to check for goal


def BFS(cost, queue, visited, goal):
    while (time.time() < time_end):
        for i in range(len(queue)):  # go over the current expanded nodes to check for goal
            if queue[i] == goal:  # if goal is reached, prints cost, path, time and exits
                ...
                print("The cost of the path found for BFS", traverse_cost)
                print("The shortest path found for BFS", path)
                print("Number of Nodes in Memory for BFS", len(queue))
                print("Number of Nodes expanded for BFS", len(visited))
                print("Total Execution Time for BFS", time.time(), " miliseconds")
                sys.exit()
            else:
                if queue[i] not in visited:
                    visited.append(queue[i])  # if goal not reached, add it to visited and expand its neighbors
        adjacent_nodes(cost, queue[0], queue[1:], visited, goal)
